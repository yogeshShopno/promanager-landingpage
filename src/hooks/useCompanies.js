...
const useCompanies = () => {
    ...
    const addCompany = async (companyData) => {
        ...
        try {
            ...
            if (companyData.company_logo && companyData.company_logo instanceof File) {
                formData.append('company_logo', companyData.company_logo);
            }

            const res = await api.post("/company_create", formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                }
            });
            ...
            if (res.data && res.data.success === false) {
                ...
                const errorMessage = res.data.message || "Failed to add company";
            ...
            // If we get here, assume success and refresh the companies list
